...
class KurlyComponent extends Component {
    ...
    onSubmit=(e)=>{
       e.preventDefault();
        ...
        //   서버에 데이터 전송시 아래의 사항에 해당하면 전송 취소
        //1. 반드시 필수 입력사항 / 입력데이터 값 빈값 여부 확인 : 공백이 하나라도 있으면 전송취소
        //2. 입력데이터 값 규칙에 어긋나는 내용이 있는 판별 빨강 오류가 있는 모든 데이터는 전송 취소
       
        //3. 그리고 약관동의[] 필수 선택사항이 하나라도 선택이 안되면 전송 취소
        //4. 중복확인 안하면 전송취소 -- 저장된 데이터랑 비교 


        // 필터(검색) 
        // '우리는 필수사항은 선택합니다.'.indexOf('필수');
        // 결과는 처음 찾은 글자위치를 인덱스번호로 출력 4
        // 못찾으면 결과가 -1 을 출력
        // indexOf('찾는문자열') ==== search('찾는문자열')
        // let cnt=0;
        // for(let i=0; i<약관동의.length; i++){
        //     // if( 약관동의[i].indexOf('필수') >=0 ){
        //     if( 약관동의[i].search('필수') >=0 ){
        //         cnt++;
        //     }
        // }
        // console.log( cnt );

        // //필터를 활용 검색 -> 새로운 배열을 생성
        // // filter()
        // let imsi1 = 약관동의.filter((item,idx)=>{
        //     if( item.indexOf('필수') !==-1  ){ //  >=0
        //         return item;
        //         //console.log(item, idx);
        //     }
        // });
        // console.log( imsi1 );
        // console.log( imsi1.length );
                                    ...
                                    <div className='right address'>
...

                                        {
                                            this.state.isPostcodeOpen && (
                                             <Postcode  onAddressHandle={this.onAddressHandle} />
                                            )
                                        }
